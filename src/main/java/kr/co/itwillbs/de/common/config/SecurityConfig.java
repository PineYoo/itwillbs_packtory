...
public class SecurityConfig {
	...
	public SecurityConfig(CustomAuthenticationSuccessHandler authenticationSuccessHandler,
			CustomAuthenticationFailureHandler authenticationFailureHandler, CustomLogoutHandler logoutHandler) {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http.csrf(csrf -> csrf.disable()).cors(cors -> cors.disable()).httpBasic(httpBasic -> httpBasic.disable())
				...
				.authorizeHttpRequests(authorizeHttpRequests -> authorizeHttpRequests
						...
						.requestMatchers("/admin/**").hasAuthority("ADMIN") // .hasRole("ADMIN") // 이녀석은 ROLE_접두사가 붙은
																			// 문자열로 표현 hasAuthority는 접두사 붙이지 않음
						// .requestMatchers("/manager/**").hasRole("MANAGER") // 스프링에서는 hasRole로 ROLE_
						// 접두사가 붙는걸 권장한다고 한다. 일단 그렇다고요. 알아 두자
				...
				.logout(logout -> logout.logoutUrl("/logout").logoutSuccessUrl("/").addLogoutHandler(logoutHandler) // 로그아웃
																													// 시
																													// 로그
																													// 기록을
																													// 위해
																													// 로그아웃핸들러
																													// 등록
						.invalidateHttpSession(true).clearAuthentication(true).deleteCookies("JSESSIONID") // 쿠키 삭제
						.permitAll())

				.sessionManagement(
						sessionManagement -> sessionManagement.sessionCreationPolicy(SessionCreationPolicy.IF_REQUIRED) // 기본
																														// 값이지만
																														// 선언해
																														// 놓자
				...
				.securityContext(securityContext -> securityContext.requireExplicitSave(true)); // 비활성화는 당연히 false 로

		return http.build();
	}

	/**
	 * 시큐리티 필터에서 제외 시키니까 html 에서 호출하는 resource들에게서 인증했다는 로그는 보이지 않는다.
	 * 근데 이게 진짜 해결책일까? 담임 선생님께 물어보장!
	 * @return
	 */
	@Bean
	 ...
	 * log.info("SecurityConfig 초기화 - 핸들러 주입 확인"); log.info("SuccessHandler: {}",
	 * authenticationSuccessHandler != null); log.info("FailureHandler: {}",
	 * authenticationFailureHandler != null); log.info("LogoutHandler: {}",
	 * logoutHandler != null); }
	 */
}
