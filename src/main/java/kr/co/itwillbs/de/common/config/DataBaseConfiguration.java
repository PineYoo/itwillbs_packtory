...
public class DataBaseConfiguration {
	...
	HikariConfig hikariConfig() {
		// application.properties 파일에서 spring.datasource.hikari로 시작하는 설정 값들을 가져와 Hikari
		// 설정 객체를 반환한다.
	...
	SqlSessionFactory sqlSessionFactory(DataSource dataSource) throws Exception {
		...
		Resource[] mapperResources = applicationContext.getResources("classpath:kr/co/itwillbs/de/mybatis/mapper/**/*Mapper.xml");
		Resource[] sqlResources = applicationContext.getResources("classpath:kr/co/itwillbs/de/mybatis/mapper/**/*Sql.xml");
		
		List<Resource> resourceList = new ArrayList<>(mapperResources.length + sqlResources.length);
		resourceList.addAll(Arrays.asList(mapperResources));
		resourceList.addAll(Arrays.asList(sqlResources));
		Resource[] allResources = resourceList.toArray(new Resource[0]);
		session.setMapperLocations(allResources);
