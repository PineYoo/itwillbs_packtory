...
//	});
    ...
    if (total === 2) {
        ...
        $('#approverName1').val(formatApproverInfo(employeeList[0]));

		$('#approver3').val(employeeList[1].id);
        $('#approverName3').val(formatApproverInfo(employeeList[1]));
    } else {
        // 그 외엔 1, 2, 3 순서대로 채움
        for (let i = 0; i < total && i < 3; i++) {
			$('#approver' + (i + 1)).val(employeeList[i].id);
            $('#approverName' + (i + 1)).val(formatApproverInfo(employeeList[i]));
        }
    }
}
